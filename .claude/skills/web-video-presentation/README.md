...
`web-video-presentation` helps an agent build a Vite + React + TypeScript presentation that behaves like a video production surface rather than a slide deck. Each click advances one narration beat, each step owns the whole 1920×1080 stage, and the stage runs full-bleed so a fullscreen 16:9 recording needs no cropping in post. A slim progress rail stays docked to the top edge — always visible, because it is part of the frame.
...
- **Full-bleed stage** — fills the viewport, so recordings need no cropping.
- **Always-on top rail** — chapter/step progress is docked to the top edge and never hides; it is composed into the frame, not dev-only chrome. Dev controls stay hover-only.
