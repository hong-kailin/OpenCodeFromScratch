...
description: 把一篇文章或口播稿，做成"看起来像视频"的点击驱动 16:9 网页演示，可选合成口播音频。流程：原始文章 → **第一步只产出口播稿 script.md，停下让用户人审**（硬节点，写死；此阶段不开 subagent 审查、不写 outline）→ 稿子认可后才产出 outline 开发计划 → 用户对齐 4 件事（outline / 主题 / 素材 / 开发模式）→ 网页开发（逐章 / 顺序 / 并行）→ 可选音频合成（provider-agnostic：内置 MiniMax mmx-cli + OpenAI TTS，可换 ElevenLabs / edge-tts / Azure / 自带 TTS）。**outline 只规划节奏与信息密度，不规划动画** —— 动画由章节开发时按 PRINCIPLES + ANTI-AI 法则即时设计。每次点击推进口播稿的一个节拍，每一步独占整屏（舞台满屏出血，录屏免裁剪），顶部常驻一条进度条显示章节 / 步进。适用场景：用网页做视频（动态 PPT 但不像 PPT）、把口播稿 / 文章变成可交互的解说、为 B 站 / YouTube / 视频号录屏教程、做有电影感的产品 / talk demo。本 Skill 沉淀的是设计方法论 + 协作流程 —— 不绑定任何特定样式 / 字体 / 颜色 —— 因此能复用到任意主题与美学。
---
...
Phase 1   内容编写
   1.1  识别用户输入
   1.2  只产出 script.md（口播稿）—— 不碰 outline
        ▼
        [Checkpoint Script]   ← 必须停（写死）。只让用户人审稿子。
                                此阶段不开 subagent 审查。稿子没认可，
                                不许写 outline / 不选主题。
        ▼
   1.3  稿子认可后，才产出 outline.md（开发计划）
   ▼
[Checkpoint Plan]      ← 必须停。对齐 4 件事：
                         outline / 主题 / 素材 / 开发模式
   ▼
Phase 2   网页开发
   2.1  脚手架（按选定主题）
...
下面这些产出，每一个**完成后必须走审查 → 修复 → 再汇报 / 推进**：

| 产出 | 审查方式 |
|---|---|
| `script.md`（Phase 1.2） | **用户人审（Checkpoint Script，写死）—— 此阶段不开 subagent / Agent Teams 审查**。理由见下 |
| `outline.md`（Phase 1.3） | [`OUTLINE-FORMAT.md`](references/OUTLINE-FORMAT.md) 自检（下方降级链） |
| 单章实现完成 | [`CHAPTER-CRAFT.md`](references/CHAPTER-CRAFT.md) 完工自检（下方降级链） |

> **为什么 `script.md` 只走人审、不开 subagent**（写死）：稿子"对不对"高度
> 依赖用户的预期与口味，agent 常常写出不符合用户预期的稿子。在一个**还没
> 和用户对齐**的稿子上做 subagent 审查是浪费——审得再细，方向错了也白搭；
> 基于它生成的 outline 更是无意义。所以 **Phase 1 一定先只出 `script.md`，
> 交用户人审，认可后才继续**。[`SCRIPT-STYLE.md`](references/SCRIPT-STYLE.md)
> 的规则仍是**写稿时**的指导（形式 / 风骨 / 念出来），只是**不再单开
> reviewer agent 去审**。

**执行方式**（适用于 `outline.md` 与单章实现；`script.md` 例外，走人审，按能力降级，**优先用更隔离的方式**）：
...
|---|---|---|
| Phase 1.2 只出 script.md | `references/SCRIPT-STYLE.md` + `article.md`（用户原文，如有） | —— |
| **Checkpoint Script 人审稿子** | —— | 无需读文件；停下等用户认可 `script.md` |
| Phase 1.3 出 outline.md（稿子认可后） | `references/OUTLINE-FORMAT.md` + `script.md` + `article.md` | —— |
...
## Phase 1 —— 内容编写（先出稿子，人审后再出 outline）

> **写死的铁律**：Phase 1 **第一步只产出 `script.md`**，然后**停下**让用户
> 人审（Checkpoint Script）。**不开 subagent / Agent Teams 审查稿子，也不
> 提前写 `outline.md`、不选主题**。只有用户**明确认可**稿子后，才进入 1.3
> 产出 outline。理由见上文「硬性自检协议」的说明。

### 1.1 识别用户输入

| 用户给的东西 | 该做的 |
|---|---|
| 原始文章（书面语 / 公众号 / 论文 / 博客） | 先只产出 `script.md`（1.2），过 Checkpoint Script |
| 直接的口播稿 / 视频脚本 | 落盘成 `script.md`（必要时按 SCRIPT-STYLE 平台化），过 Checkpoint Script |
| 啥都没有，只说"帮我做个 X 主题的视频" | **反问**：先给一段素材或大纲。Skill 不替用户构思内容 |

### 1.2 只产出 script.md

**这一步只做一件事**：按 [`references/SCRIPT-STYLE.md`](references/SCRIPT-STYLE.md)
的规则把 article 转成保持原文语言的平台化口播稿，落盘 `script.md`。
**保留 `article.md` 不删**——它是后面 outline 写信息池、章节实现画面时的
细节源（双源原则）。

**这一步不许做的事**（写死）：

- ❌ 不写 `outline.md`（等稿子认可后的 1.3）
- ❌ 不开 subagent / Agent Teams 审查稿子（改由用户人审）
- ❌ 不选主题 / 不列素材 / 不问开发模式（都留到 Checkpoint Plan）

写稿时 [`SCRIPT-STYLE.md`](references/SCRIPT-STYLE.md) 的三层要求（形式 /
风骨 / 念出来）仍要遵守——那是**写**的指导；只是**不再单开 reviewer
agent 去审**，把审的环节交给用户。落盘后直接进 Checkpoint Script。

---

## Checkpoint Script —— 只审稿子（**硬节点 · 写死**）

`script.md` 落盘后**必须停**。这一节点**只**让用户审一件事：稿子。**不要**
在这里塞 outline / 主题 / 素材 / 模式——那些是下一个 checkpoint 的事。

### 总结模板（骨架，agent 按情况填充）

```
口播稿写好了：📄 script.md（{X} 字 / ~{T} 分钟 / {N} 个节拍）

先只请你审稿子——方向对了我们再往下做：
  □ 内容对不对？有没有漏掉你想讲的、或多了你不想要的？
  □ 语气 / 风格是不是你要的（B站 / 抖音 / 知乎 / YouTube…）？
  □ 节拍（--- 切分）顺序对不对？
  □ 念出来顺不顺口？哪句别扭？

可以直接编辑 script.md，或口头告诉我改哪。
稿子 OK 了告诉我"稿子可以了 / 继续"，我再去切 outline（章节 + 信息池）。
```

收到反馈后：
- 用户要改：直接改文件或口头描述 → agent 改 → 再确认
- **稿子没得到明确认可，不许进入 1.3**（不写 outline、不选主题、不列素材）

### 1.3 稿子认可后 —— 产出 outline.md

用户**明确认可** `script.md` 后，按 [`references/OUTLINE-FORMAT.md`](references/OUTLINE-FORMAT.md)
...
**落盘后走 outline 自检再进 Checkpoint Plan**：按上文「硬性自检协议」对
`outline.md` 执行（优先 Agent Teams → subAgent → 自检），按结论修复完成
后再进入 Checkpoint Plan。

---

## Checkpoint Plan —— 4 件事一次对齐（**硬节点**）

稿子已在 Checkpoint Script 审过并认可；`outline.md` 写完后必须停下来。
**用户在这一个节点同时确认 4 件事**（稿子若还想微调，也可以在这里一并说）。
...
接下来一次对齐 4 件事（稿子上一步已认可，若还想微调也在这里一并说）：
...
收到反馈后：
- outline 要改（稿子若还要微调也一起）：直接编辑文件，编辑完 ping 一次（或口头描述 agent 改）
...
| 5 | 常驻顶部进度条 | 顶边贴一条进度条，**永不隐藏**（它是画面的一部分）；其余开发控件 opacity 0 |
| 6 | 舞台满屏出血 | 舞台铺满视口（scale 边距 = 0），进度条**浮在**舞台上而非挤走它 —— 挤走就破 16:9，又得后期裁 |
...
| [`references/OUTLINE-FORMAT.md`](references/OUTLINE-FORMAT.md) | Phase 1.3 必读（稿子认可后） | outline.md 字段 spec、命名约定、章节切分、信息池 |
