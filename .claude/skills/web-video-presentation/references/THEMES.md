...
> ⚠️ **主题的 `tokens.css` 必须保持"无层"（不要包 `@layer`）。**
> `base.css` 和主题的 `tokens.css` 在**同一批性格旋钮**上都写了 `:root`
> （`--r-card` / `--rule-w` / `--hero-num-*` / `--dur-*` / `--ease-overshoot`
> / `--stage-pad-*` / `--shadow-stage`），特异性完全相同。base.css 把自己
> 的 `:root` 放进了 `@layer defaults`，而**无层规则永远赢过有层规则**——
> 主题因此不依赖导入顺序就能赢。你要是把 `tokens.css` 也包进某个 layer，
> 这条保证就失效了，主题会被 base 的默认值悄悄覆盖（圆角卡片 / 1px 细线
> / 斜体 400 的 hero 数字 / 回弹缓动），而且**不报任何错**。
...
### 可选的 chrome 覆盖（顶部常驻进度条）

顶部进度条**永不隐藏，会被录进成片**，所以它是构图的一部分，按主题的审美调。

> ⚠️ **它有自己的一套颜色 token，别拿 `--text` / `--text-mute` 给它上色。**
> 那几个是为了在 `--surface`（舞台底色）上出对比而调的：浅色主题里 `--text`
> 是近黑，铺到 bar 的深色 `--chrome-bg` 上就是**黑底黑字**，当前章标题直接
> 消失。以前没人发现，是因为 bar 默认隐藏、没人读过它。

| token                 | 默认值                    | 作用                                              |
| --------------------- | ------------------------- | ------------------------------------------------- |
| `--chrome-h`          | `40px`                    | bar 高度。**必须 ≤ `--stage-pad-y`**，否则压到内容 |
| `--chrome-bg`         | `rgba(0,0,0,0.88)`        | bar 底色                                          |
| `--chrome-blur`       | `blur(20px)`              | backdrop-filter；扁平 / 无模糊主题设 `none`        |
| `--chrome-text`       | `#ffffff`                 | 当前章标题                                        |
| `--chrome-text-mute`  | `rgba(255,255,255,0.55)`  | 其余章标题                                        |
| `--chrome-text-faint` | `rgba(255,255,255,0.28)`  | 未走到的 pip / 章节号                              |
| `--chrome-accent`     | `var(--accent)`           | 高亮边框 / 已走过的 pip                            |
| `--chrome-border`     | `rgba(255,255,255,0.12)`  | bar 底边分隔线                                    |

默认这套（深底 + 浅字）在**任何**主题上都读得清，不改也不会错。
...
- accent 圆点在发光但不爆。
- 斜体强调有可读的背景。
- 顶部常驻进度条读得清：当前章标题**没有糊在 bar 底色里**（别用 `--text` 给它上色，见 `--chrome-*`），高亮是 accent 色。
