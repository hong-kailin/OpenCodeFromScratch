...
- **16:9 满屏出血舞台**：内容设计在 1920×1080 上，外层 transform scale
  铺满视口（`useStageScale` 边距 = 0）—— **没有响应式断点**。全屏（F11）
  录 16:9 屏幕时舞台 1:1 对齐物理像素，**后期不用裁**
- **舞台居中 + 大留白**：上下左右四边都让出至少 80px 的安全区
- **常驻顶部进度条**：贴着顶边，**永不隐藏** —— 它会被录进画面，是构图的
  一部分（让观众知道讲到哪了），支持点击跳转章节。它**浮在**舞台之上，
  占的正是顶部那条安全区：
  **章节内容不要顶到距顶边 `--chrome-h`（默认 40px）以内**
...
- **`clip-path` 擦入的终态永远不能是 `inset(0 0 0 0)`** —— 这是最容易中招的一条。
  `inset(0)` 不是"不裁"，是"**严格裁到 border box**"；配上 `fill-mode: both`
  它在动画结束后**永久生效**，把所有溢出 border box 的字形墨迹削平：
  `y / g / p / j` 的降部、CJK 的字底、斜体外挑、以及 offset 投影。
  `line-height` 越紧（`line-height: 1` 或比 `font-size` 还小的 hero 数字）
  溢出越多、削得越狠。**页面静止时看不出是动画的锅 —— 它看起来就像"字体坏了"。**

  正确写法（只有**扫掠的那条边**动，另外三边在两个关键帧里都钉在 `-9999px`，
  值相同 → 不参与插值 → 不影响时序，只保证永远不裁）：

  ```css
  @keyframes x-wipe-k {
    from { clip-path: inset(-9999px 100% -9999px -9999px); }  /* 右边扫 */
    to   { clip-path: inset(-9999px -48px -9999px -9999px); } /* 扫过头一点，免得留发丝 */
  }
  ```

  **别用 `em` 当出血量**：`em` 按元素自己的 `font-size` 解析，一个 16px 的
  容器只能拿到 8px 出血 —— 盖不住 250px hero 的墨迹，也盖不住 8px 的实心投影。
  同理，`overflow: hidden` 的容器也会削字，但它常常是**有意**的遮罩，
  改之前先确认意图。

- **删 CSS 前先查 `@keyframes` 有没有别人在用**：工具类（`.x-drop`）没人用
  ≠ 它引用的 keyframe（`x-drop-k`）没人用。直接写
  `animation: x-drop-k ...` 的规则不会出现在"class 有没有被 JSX 用到"的
  检查里。删掉 keyframe 不会报错，只会让那些入场**静默失效**。
