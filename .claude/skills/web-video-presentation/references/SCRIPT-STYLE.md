...
> ⚠️ **写死的流程**：`script.md` 写完后**不开 subagent / Agent Teams 审查**，
> 而是**直接交用户人审**（SKILL.md 的 Checkpoint Script）。下面的三层清单
> （形式 / 风骨 / 念出来）是你**写稿时自己要过一遍的自查**，不是单开
> reviewer agent 的审查环节——把"审"这件事留给用户。
>
> **为什么**：稿子对不对高度依赖用户预期，agent 常写出不合预期的稿子；
> 在没和用户对齐的稿子上做 subagent 审查、再据此生成 outline，都是浪费。
>
> **顺序（写死）**：写 `script.md` → 自己过一遍下面三层清单、就地改 →
> **停下交用户审** → 用户认可后**才**产出 outline（**禁止**写完稿子直接
> 顺手产 outline）。
...
### 5. 停下交用户审 —— 认可后才产出 outline

`script.md` 落盘 + 自己过完三层清单后，**必须停下来交用户人审**（SKILL.md
的 Checkpoint Script）。**不要**在同一次思考里顺手把 `outline.md` 也写了。

- 用户要改稿子 → 改 → 再确认
- 用户**明确认可**稿子后，**才**按 [`OUTLINE-FORMAT.md`](OUTLINE-FORMAT.md)
  产出 `outline.md`（章节切分 + 每步屏幕内容 + 章节级信息池），然后进入
  Checkpoint Plan 对齐 4 件事（outline / 主题 / 素材 / 开发模式）。

> **为什么先审稿再出 outline**（写死）：稿子对不对高度依赖用户预期，agent
> 常写出不合预期的稿子。在没对齐的稿子上审查、再据此生成 outline，都是
> 浪费——先让用户把稿子的方向定死，后面的 outline 才有意义。详见 SKILL.md
> 「Phase 1」与「硬性自检协议」。
