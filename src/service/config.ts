...
// 阶段 11 教学代码：ConfigService——用 Effect Service + Layer 实现依赖注入
//
// 解决的痛点（阶段 10.1）：
// - 之前 loadConfig() 每次调用都读一遍 opencode.json
// - 现在用 ConfigService：Layer 启动时读一次文件并缓存，所有消费者共享同一份
//
// Service 三件套（名字都是我们自己起的，不是 effect 关键字）：
// 1. ConfigServiceApi  -- 声明"这个服务能做什么"（能力清单，一个 interface）
...
// 所以读文件只发生一次，读到的 config 在内存中缓存，所有消费者共享。
// 类比：工具管理处——工具造一次，发给所有需要的工人。
export const configLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // ConfigService.of(...) 是 Context.Service 自动给类加的静态方法
    // 作用：把实现对象包装成服务实例，存进 Context
    // get() 返回 Effect.succeed(config)——config 已经缓存在闭包里，直接给
