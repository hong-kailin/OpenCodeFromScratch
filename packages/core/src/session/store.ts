...
// 阶段 16.6 教学代码：SessionStore Service——session + message 存储服务化
//
...
//   2. 都是模块级函数 + 直接 import 模块级 db，无法替换实现
//   3. 没有"存储"这个概念的边界——谁需要存储就从哪 import 函数
//
// 现在（阶段 16.6）：SessionStore Service（三件套）
...
//   同时：16.2 的过渡桥接 db 导出在这里被真正取代，可以删掉了。
//
  ...
  Effect.gen(function* () {
    ...
    return SessionStore.of({
      ...
      create: Effect.fn("SessionStore.create")(function* (title?: string) {
        ...
        // insert 返回 Promise，用 Effect.promise 桥接
