...
// 阶段 16.7 教学代码：SystemContext Service——组装 system prompt
//
...
//
// 现在（阶段 16.7）：SystemContext Service（三件套）
...
//   后续阶段如果系统提示组件变多，再演进成 registry 模式。
...
// 返回 Effect<string> 而不是裸 string——为未来异步组件留余地（现在内部是同步 fs）
