...
//
// 阶段 16.4 改动：execute 从 Promise 升级为 Effect（对照 opencode 的真实签名）。
// 之前：
//   execute(args): Promise<string>          —— 工具直接调 Bun/file，拿不到 Context
// 现在：
//   execute(args): Effect.Effect<string>    —— 工具内部 yield* FileSystemService 取依赖
// 为什么：阶段 16.3 把文件操作收进 FileSystem 服务，工具必须能"从 Context 取服务"。
//   Promise 函数体内没有 Context，Effect 函数体里有（yield* 就是取服务的语法）。
//   第三泛型 R 声明"这个工具需要哪些服务"——类型系统强制提供，编译期报错。

import { Effect, Schema } from "effect"

// 一个工具的完整定义（泛型 Parameters：本工具的参数 Schema）
// 泛型 R：本工具 execute 需要哪些服务（如 FileSystemService）
