...
// 阶段 16.3 改动：execute 从 Promise 改 Effect（统一工具签名；bash 不依赖 FileSystem）
...
const execute = (args: Schema.Schema.Type<typeof Parameters>) =>
  Effect.gen(function* () {
    ...
    // Bun.spawn 是同步创建 + 异步读取，包在 Effect.promise 里（类比 asyncio 桥接）
    // 为什么用 Effect.promise：Bun.spawn 返回 Promise 接口（proc.stdout 等），
    // 和 Effect 不直接兼容，需要桥接（阶段 14 也这么桥接过 response.body）
