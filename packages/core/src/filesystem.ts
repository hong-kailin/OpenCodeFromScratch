...
//
// 解决的问题（为什么需要这个服务）：
//   之前（阶段 4-15）：read/write/edit/glob/grep 工具各自直接调 Bun.file / Bun.Glob
//   问题：
...
//   16.3 先让服务方法对齐这个签名（Promise），16.4 工具 Effect 化时
//   工具内部用 Effect.promise 桥接。服务三件套结构不变——
//   Promise 还是 Effect 只是返回类型的选择。
//
// 对照 opencode: packages/core/src/filesystem.ts
//   opencode 的 FileSystem 服务有 read/list/find/glob/grep + location/realPath
//   安全检查（防路径逃逸）+ 返回 Uint8Array + mime 类型。
//   我们简化：只封装当前工具需要的 read/exists/write/glob/grep。
...
export const fileSystemLayer = Layer.effect(
  ...
  Effect.sync(() =>
    FileSystemService.of({
      ...
      // 跳过 node_modules 和 opencode（原来散在各工具里的硬编码，集中到这里管理）
      glob: async (pattern) => {
