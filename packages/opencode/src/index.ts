...
// 跑法：
//   bun run packages/opencode/src/index.ts run              # 交互模式
//   bun run packages/opencode/src/index.ts run "你好"       # 非交互模式
//   bun run packages/opencode/src/index.ts run -c           # 恢复上次会话
//   bun run packages/opencode/src/index.ts run -s ses_xxx   # 恢复指定 session
//
// 重构前（阶段 9）：
...
//
// 阶段 16.6 改动：上层接入 SessionStore 服务
//   之前：直接调用模块级函数（createSession()、listSessions()、saveMessage() 等）
//   现在：yield* SessionStore——从 Context 取服务，存储逻辑全部走服务
//   index.ts 瘦身：不再 import 具体存储函数，只 import 服务 tag 和 Layer
...
// sessionStoreLayer 依赖 DatabaseService（16.6），所以也要先喂
//
...
// 16.6：SessionStore 依赖 Database，先喂 databaseLayer
