...
本课有可执行示例：`src/tui-demo/01-reactive-demo.ts`

```bash
bun run src/tui-demo/01-reactive-demo.ts
```

三个演示对照本文的三个部分：命令式的 bug（忘了手动刷新）、响应式自动更新、Excel 类比。跑之前确认项目根目录有 `bunfig.toml`（原因见 03 课的 Debug 说明）。
